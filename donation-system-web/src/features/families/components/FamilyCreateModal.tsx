...
const FamilyCreateModal: React.FC<FamilyCreateModalProps> = ({ show, onClose, onSave, loading = false }) => {
  ...
  const validate = (formData: CreateFamilyRequest) => {
  ...
  const handleChange = (
    ...
    setForm((prev) => ({
      ...prev,
      [name]: name === 'monthlyIncome' ? (value === '' ? undefined : Number(value)) : value,
    }));
  };
    ...
    >
      <div style={{ borderRadius: '16px', overflow: 'hidden', background: '#fff' }}>
        <Modal.Header style={{ borderBottom: '1px solid var(--border)', padding: '20px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#fff' }}>
          <div>
            <Modal.Title style={{ fontSize: '18px', fontWeight: 700, color: 'var(--text-main)', marginBottom: '2px' }}>Cadastrar Nova Família</Modal.Title>
            <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>Preencha as informações do responsável para cadastrar a família no sistema</div>
          </div>
