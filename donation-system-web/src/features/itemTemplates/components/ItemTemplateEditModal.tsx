...
interface ItemTemplateEditModalProps {
  show: boolean;
  onClose: () => void;
  onSave: (id: string, data: UpdateItemTemplateRequest) => void;
  loading?: boolean;
  initialData: ItemTemplateResponse | null;
}

const ItemTemplateEditModal: React.FC<ItemTemplateEditModalProps> = ({ show, onClose, onSave, loading = false, initialData }) => {
  const [form, setForm] = useState<UpdateItemTemplateRequest>({
    name: '',
    categoryId: '',
    needGroupId: '',
    isPerishable: false,
  ...
  useEffect(() => {
    if (initialData && show) {
      setForm({
        ...
        suitableForAutoSuggestion: initialData.suitableForAutoSuggestion ?? true,
        requiresManualAnalysis: initialData.requiresManualAnalysis ?? false,
        defaultUnitOfMeasure: initialData.defaultUnitOfMeasure || '',
  ...
  const validate = (formData: UpdateItemTemplateRequest) => {
    ...
    <Modal show={show} onHide={onClose} centered size="lg" backdrop="static">
      <div className="p-3">
        <Modal.Header closeButton className="border-0 pb-0">
          <Modal.Title className="fw-bold">Editar Modelo de Item</Modal.Title>
        </Modal.Header>
        ...
        <Modal.Footer className="border-0 pt-0">
          ...
          <button className="btn-primary-custom" onClick={handleSubmit} disabled={loading}>
            {loading ? 'Salvando...' : <><FiSave className="me-1" /> Salvar Alterações</>}
          </button>
        </Modal.Footer>
      </div>
    </Modal>
...
export default ItemTemplateEditModal;
